...
import { withErrorHandling, withSyncErrorHandling, createError, logger } from "../core/agent";
 ...
 */
export function cleanupFiles(directory: string, pattern: RegExp | string): number {
 ...
 */
export function getFilesInDirectory(directory: string, pattern?: RegExp | string): string[] {
  if (!fs.existsSync(directory)) {
    return [];
  }
  ...
  return withErrorHandling(
    async () => {
      if (!fs.existsSync(envPath)) {
        throw createError(`Environment file not found: ${envPath}`, "Environment file reading");
      }
 ...
 */
export function validateEnvVars(envVars: Record<string, string>, required: string[]): void {
  const missing = required.filter(varName => !envVars[varName]);
  
  if (missing.length > 0) {
    throw createError(`Missing required environment variables: ${missing.join(", ")}`, "Environment validation");
  }
}
 ...
 */
export async function loadEnvironmentConfig(envPath: string = '.env'): Promise<Record<string, string>> {
...
export function getEnvVar(name: string, fallback?: string): string {
  ...
  if (value === undefined && fallback === undefined) {
    throw createError(`Environment variable ${name} is required`, "Environment validation");
  }
...
export function getNumberEnvVar(name: string, fallback?: number): number {
  ...
  if (isNaN(parsed)) {
    throw createError(`Environment variable ${name} must be a number, got: ${value}`, "Environment validation");
  }
