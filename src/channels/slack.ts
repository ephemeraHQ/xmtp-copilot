...
class XMTPCopilot {
  ...
  private setupEventHandlers(): void {
    ...
    this.app.message(async ({ message, say, client }) => {
      ...
      const existingController =
        this.sessionManager.activeControllers.get(sessionKey);
  ...
  private async processMessage(
    ...
    try {
      ...
      for await (const chunk of this.claudeHandler.streamFromClaude(
        session.messages,
        abortController,
      )) {
        ...
        if (!responseMessageTs) {
          ...
          console.log(
            "✅ Sent initial response chunk, message TS:",
            responseMessageTs,
          );
        } else {
          // Update the response
    ...
    if (!responseMessageTs && !fullResponse.trim()) {
      ...
      await say({
        text: formatTextForSlack(
          "Hello! I received your message but had trouble processing it. Please try again.",
        ),
        thread_ts: threadContext,
      });
    }
