...
agent.on("text", async (ctx) => {
  ...
  const sessionKey = sessionManager.getSessionKey(senderAddress, conversationId);
  ...
  try {
    ...
    for await (const chunk of claudeHandler.streamFromClaude(session.messages, abortController)) {
    ...
    if (error.name !== 'AbortError') {
      ...
      await ctx.sendText('Sorry, I encountered an error while processing your message.');
    }
  } finally {
