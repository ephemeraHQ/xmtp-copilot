import { CliParam, CliConfig, parseCliArgs, generateHelpText as generateHelpTextShared } from './cli-utils';
...
export function generateHelpText(
  ...
  return generateHelpTextShared(skillName, description, usage, allParams, examples);
}
...
export function validateMutuallyExclusive(
  ...
  groups.forEach(group => {
    ...
    if (present.length > 1) {
      throw new Error(`Cannot use these parameters together: ${present.join(', ')}`);
    }
  });
}
