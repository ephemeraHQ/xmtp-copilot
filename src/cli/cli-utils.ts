 ...
 */
export function parseCliArgs(args: string[], params: Record<string, CliParam>): CliConfig {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    for (const [key, param] of Object.entries(params)) {
      if (param.flags.includes(arg)) {
        if (param.type === 'boolean') {
          config[key] = true;
        } else if (param.type === 'string' || param.type === 'number') {
          if (nextArg && !nextArg.startsWith('--')) {
            config[key] = param.type === 'number' ? parseInt(nextArg, 10) : nextArg;
 ...
 */
export function validateMutuallyExclusive(config: CliConfig, exclusiveGroups: string[][]): void {
  for (const group of exclusiveGroups) {
    const provided = group.filter(param => config[param] !== undefined);
    if (provided.length > 1) {
      throw new Error(`Parameters are mutually exclusive: ${provided.join(', ')}`);
    }
  }
}
